...
function Dashboard() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => fetchReports(searchQuery), 300);
  ...
  const filteredReports = reports.filter((r) => {
    const name = (r.patient_info?.patientName || "").toLowerCase();
    ...
    <div className="min-h-screen bg-[#0f172a] text-slate-200 font-sans selection:bg-indigo-500/30 overflow-x-hidden">
      ...
      <header className="container mx-auto px-6 py-8 flex justify-between items-center border-b border-slate-800/50 backdrop-blur-md sticky top-0 z-50">
        ...
        <div className="flex items-center gap-6">
          {/* Doctor Info Badge */}
          {doctor && (
            <div className="hidden md:flex items-center gap-3 bg-slate-800/50 border border-slate-700/50 rounded-2xl px-4 py-2.5">
              <div className="w-8 h-8 rounded-full bg-indigo-600/20 flex items-center justify-center text-sm">
                👨‍⚕️
              </div>
              <div>
                <p className="text-xs font-semibold text-slate-300 leading-none">{doctor.fullName}</p>
                <p className="text-[10px] text-slate-500 mt-0.5">{doctor.hospital} • {doctor.doctorId}</p>
              </div>
            </div>
          )}
          ...
          <button
            onClick={logout}
            className="text-xs font-semibold text-rose-400 hover:text-rose-300 transition-colors bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 rounded-xl px-4 py-2 cursor-pointer"
          >
            Sign Out
          </button>
        </div>
      ...
      <main className="container mx-auto px-6 pt-16 pb-24 grid lg:grid-cols-12 gap-12 items-start">
        ...
        <section className="lg:col-span-8 space-y-8 order-1 lg:order-2">
          <div className="bg-white/5 backdrop-blur-2xl border border-white/10 p-8 rounded-[2rem]">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
              <div>
                <h3 className="text-lg font-bold text-white">Your EEG Reports</h3>
                <p className="text-xs text-slate-500 mt-1">All analyses generated under your account</p>
              </div>
              <input
                type="search"
                ...
                className="bg-slate-900/60 border border-slate-700 rounded-xl px-4 py-2 text-sm w-full md:w-64"
              />
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                ...
                <tbody>
                  {reportsLoading ? (
                    <tr>
                      <td colSpan={5} className="py-8 text-center text-slate-500">Loading reports…</td>
                    </tr>
                  ) : filteredReports.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="py-8 text-center text-slate-500">
                        No reports yet. Run an EEG analysis below.
                      </td>
                    </tr>
                  ) : (
                    filteredReports.map((r) => (
                      <tr key={r.id} className="border-b border-slate-800/50 hover:bg-slate-800/20">
                        <td className="py-3 pr-4 font-medium text-slate-200">
                          {r.patient_info?.patientName || "N/A"}
                        </td>
                        <td className="py-3 pr-4 text-slate-400">{formatDate(r.createdAt)}</td>
                        <td className={`py-3 pr-4 font-semibold ${
                          r.prediction === "Healthy" ? "text-emerald-400" : "text-rose-400"
                        }`}>
                          {r.prediction}
                        </td>
                        ...
                        <td className="py-3">
                          <button
                            type="button"
                            onClick={() => navigate(`/report-form/${r.id}`)}
                            className="text-xs font-semibold text-indigo-400 hover:text-indigo-300 bg-indigo-500/10 px-3 py-1.5 rounded-lg"
                          >
                            {r.status === "Sent" ? "View Report" : "Generate Final Report"}
                          </button>
                        </td>
                      </tr>
                    ))
                  ...
                  result.includes("Healthy") 
                    ...
                    <div className="space-y-2">
                      ...
                      {lastReportId && (
                        <button
                          type="button"
                          onClick={() => navigate(`/report-form/${lastReportId}`)}
                          className="mt-4 text-sm font-semibold text-indigo-400 bg-indigo-500/10 px-4 py-2 rounded-xl hover:bg-indigo-500/20"
                        >
